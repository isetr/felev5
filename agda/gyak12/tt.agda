...
postulate
  ⊤ : Set
  tt : ⊤
  Ind⊤ : ∀{i}(P : ⊤ → Set i)(p : P tt)(x : ⊤) → P x
  ttβ : ∀{i}{P : ⊤ → Set i}{p : P tt} → Ind⊤ P p tt ≡ p
  {-# REWRITE ttβ #-}
...
postulate
  ...
  IndΣ   : ∀{i j k}{A : Set i}{B : A → Set j}(P : Σ A B → Set k)
           (p, : (a : A)(b : B a) → P (a , b))(w : Σ A B) → P w
  ,β     : ∀{i j k}{A : Set i}{B : A → Set j}{P : Σ A B → Set k}
           {p, : (a : A)(b : B a) → P (a , b)}{a : A}{b : B a}
         → IndΣ P p, (a , b) ≡ p, a b
  {-# REWRITE ,β #-}

proj₁ : ∀{i j}{A : Set i}{B : A → Set j}(w : Σ A B) → A
proj₁ {A = A} w = IndΣ (λ _ → A) (λ a _ → a) w

proj₂ : ∀{i j}{A : Set i}{B : A → Set j}(w : Σ A B) → B (proj₁ w)
proj₂ {B = B} w = IndΣ (λ w → B (proj₁ w)) (λ _ b → b) w
...
------------------------------------------------------------------------------
-- postulates for handling equality
------------------------------------------------------------------------------
...
-- TODO: use Ind... notation everywhere
