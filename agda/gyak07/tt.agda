...
postulate
  ...
  true  : 𝔹
  false : 𝔹
  if[_]_then_else_ : ∀{i}(P : 𝔹 → Set i)(b : 𝔹)(pt : P true)(pf : P false) → P b
  trueβ            : ∀{i}{P : 𝔹 → Set i}{pt : P true}{pf : P false} → if[ P ] true then pt else pf ≡ pt
  falseβ           : ∀{i}{P : 𝔹 → Set i}{pt : P true}{pf : P false} → if[ P ] false then pt else pf ≡ pf
  {-# REWRITE trueβ #-}
  {-# REWRITE falseβ #-}
